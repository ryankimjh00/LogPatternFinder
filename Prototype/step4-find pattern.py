...
def find_all_repeating_patterns(file_path, output_file):
    with open(file_path, 'r') as file:
        ...
        processed_text = []
        for word in text:
            if re.match(r'\d+:', word):  # 숫자 뒤에 콜론(:)이 오면 줄바꿈 추가
        ...
        for i, lcp in enumerate(lcp_arr):
            if lcp >= 4 and lcp < 15:  # 4개 이상의 단어로 이루어진 패턴만 선택
...
file_path = "./conclusion/3.thread-scaled.txt"
output_file = "./conclusion/4.patterns.txt"

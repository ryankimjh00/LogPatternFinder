...
def find_all_repeating_patterns(file_path, output_file):
    ...
    with open(file_path, 'r') as file:
        ...
        for i, lcp in enumerate(lcp_arr):
            if lcp >= 6 and line_indices[suffix_arr[i]] != -1:  # 빈 패턴이 아닌 경우이고 시작 인덱스가 -1이 아닌 경우
    ...
    with open(output_file, 'w') as out_file:
        for pattern, indices in sorted_patterns:
            if len(indices) >= 2:
                ...
                out_file.write("Start Line Indices: " + ", ".join([f"{index[0]+1}({index[1]+1})" for index in indices]) + "\n")

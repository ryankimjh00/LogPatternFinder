...
def find_all_repeating_patterns(file_path, output_file):
    ...
    with open(file_path, 'r') as file:
        ...
        for i, lcp in enumerate(lcp_arr):
            if lcp >= 7 and line_indices[suffix_arr[i]] != -1:  # 빈 패턴이 아닌 경우이고 시작 인덱스가 -1이 아닌 경우
    ...
    sorted_patterns = sorted(repeating_patterns.items(), key=lambda x: (min([idx[0] for idx in x[1]]), extract_number_after_colon(x[0]), len(x[0]), x[0]))

    with open(output_file, 'w') as out_file:
        for pattern, indices in sorted_patterns:
            if len(indices) >= 2:
                out_file.write(f"Pattern: \n{pattern}\n")
                out_file.write("Start Line Indices:\n")
                for index in indices:
                    start_line_index = index[0]
                    out_file.write(f"Line {start_line_index + 1}: ")
                    relative_positions = sorted([i + 1 for i in range(index[1])], key=lambda x: processed_text[start_line_index + x - 1])  # 패턴의 각 줄을 processed_text에서 찾아 정렬
                    out_file.write(", ".join(map(str, relative_positions)) + "\n")  # 시작 라인의 인덱스와 각 줄의 상대적인 위치를 출력

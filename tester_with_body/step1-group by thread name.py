...
def group_patterns_by_word(file_path):
    ...
    for line in lines:
        ...
        matches1 = re.findall(r'\b((?!SQU-OTO)\w+(?:-\w+)+)\b', line)
        matches2 = re.findall(r'\b((?!RQU-OTO)\w+(?:-\w+)+)\b', line)
...
def write_grouped_patterns(grouped_patterns, output_file):
    with open(output_file, 'w') as file:
        ...
        for pattern, lines in sorted(grouped_patterns.items(), key=lambda x: x[1][0]):
            for line in lines:
                file.write(f"{line}\n")
            file.write("\n")
...
output_file = "/mnt/c/Log Pattern/tester_with_body/conclusion/1.thread-grouping.txt"

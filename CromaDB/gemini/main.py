...
def main(
    ...
    while True:
        ...
        results = collection.query(
            query_texts=[query], n_results=5, include=["documents", "metadatas", "scores"]
        )

        sources_and_scores = "\n".join(
            [
                f"{result['filename']}: line {result['line_number']}, Similarity Score: {result['score']}"
        ...
        # Output, with sources and scores
        print(response)
        print("\n")
        print(f"Source documents and similarity scores:\n{sources_and_scores}")
        print("\n")
